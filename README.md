...
Skeletons — the **standard icebow (X-Bow 3.5) control** deck (Classic 1v1; the card levels are set to a
...
lock the enemy princess tower), the **Evolved Knight** as a cheap mini-tank to defend and tank for the X-Bow, and the
...
- **Optional object detector (Stage 3).** A YOLO detector trained on your own frames
  ...
  side has its own tooling: `label-queue` ranks the unlabelled backlog by how confused
