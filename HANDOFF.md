...
*RL LAYER BUILT AND READY TO START (a), L68 foreman run `.foreman/rl-royale/` (owner 2026-09-23: "set everything up so training can start on a whim"). NO long run has been started.* Plan `scratchpad/gauntlet/L68/rl_plan.md` (port of the owner-reviewed E1 design §AN to RoyaleSim); runbook `scratchpad/gauntlet/L68/rl/RUNBOOK.md`. Start: `research/ext/Royale/.venv/Scripts/python.exe -m pipeline.rl_royale --config pipeline/rl_royale.yaml --run <name>` (`--smoke` first, ~10-20 min; `--resume`; STOP file in the run dir). Pieces, each blind-verified (same model family, independent context) and lead-accepted: (1) `e1_eval.run_batch` batched live + tempered `sample` decide with trajectory recording (1587c1a; live parity 58/58; recorded log-probs recomputed from stored inputs to <= 1e-5 over 3,043 decisions); (2) `pipeline/rl_gate.py` paired init-vs-candidate gate scorer, entry-clustered CI, real engine or RoyaleSim (96dca0b); (3) `pipeline/rl_royale.py` PPO-clip from v6aug_s1, group leave-one-out baseline, terminal reward, KL(pi||init) on all three heads with adaptive beta around an OWNER-SET `kl_target` (never raised by the trainer), per-match loss weights, 3 actor processes + GPU learner, checkpoints every 5 updates + `_latest` every update, clean non-finite stop, resume (d1a78de..8dbeb5f). Stop rules: plays/min band, KL at the beta clamp, pro-agreement TRIPWIRE (v3-VAL cell < init - 1 pp AND held-out screen delta <= 0) + hard stops (cell/card < init - 3 pp, gate_bal < init - 0.05), 3-seed held-out screen (58 x k0-2) stop only if delta <= -10 pp AND CI upper < 0, entropy < 50% of init, exploit guards (outlived-script, <=10-delivered, ghost refused >= max(2x, +1.0)). INIT (v6aug_s1) v3 VAL clean: cell 0.2073, card 0.6394, gate_bal 0.7636; init RoyaleSim held-out screen 0.747 (174 matches). MEASURED under ~90% background CPU: one default update (E16 x G4 = 64 matches, ~24.6k decisions) ~135 s, screen ~150-300 s; on-policy check at update 0 max|ratio-1| 7.8e-6, KL ~1e-12; screen noise with the policy barely moved (KL_cell <= 0.006): 35-46 of 174 pairs flip but the clustered delta stays within +-1.2 pp, CI ~+-7 pp. Only 299 / 1,505 train ghosts load in RoyaleSim, so opponents are REVISITED -- the held-out screen is the pool-fitting guard. Not built: E1 4.2.4 (won-match length flag), 4.2.5 (per-ghost flip list). Untested: a full-config long run; actor restart on the real trainer; the tripwire/hard stops firing live. Owner-requested smoke 2026-09-24 (`L68/rl/smoke_20260924.out`): SMOKE PASS, exit 0, no leftover processes; numbers identical to the verification smokes (deterministic).

*GENERALIST QUESTION (owner 2026-09-24: one model for many decks, top-10k deck pool, IL from huggingface VanguardX101/IL_Replay, then RL + multi-deck self-play). The dataset IS the FirstLight 252k set already used in §5cs.95-96 (local: scratchpad/gauntlet/L67/hf/replays, 52 parts). Census (a), `L68/generalist/deck_census.py` -> deck_census.json:* 252,238 replays = 504,476 deck-sides; 47,800 distinct base decks (52,566 with evo/hero forms); 180 distinct card keys. Concentrated: top 100 decks = 41.4% of sides, top 1,000 = 73.7%, top 10,000 = 91.5%. Sides per deck at rank 1 / 10 / 100 / 1,000 / 5,000 / 10,000: 15,398 / 4,648 / 698 / 58 / 6 / 2; decks with >=50 sides 1,121, >=20 2,233, >=10 3,767. RoyaleSim can run (every base card loaded) 26 of the top 100, 183 of the top 1,000, 1,553 of the top 10,000 (21.5% of all sides); top missing cards: lightning, mother-witch, vines, miner, royal-ghost, heal-spirit, rage, elixir-collector, graveyard, cannon-cart, goblin-hut, goblinstein, barbarian-barrel. Caution carried from §5cs.96: 603 icebow replays from this same set moved exact-cell +0.11 pp vs a predicted +0.7 (card +1.96 pp). BUILT + VERIFIED 2026-09-24 (L68 s/v): `pipeline/dataset_gen.py` (card-identity rows for any deck; starter `icebow/data/pipeline/gen_dataset_starter.npz` = 986,928 rows from corpus_v6 icebow+hogeq incl. every OPPONENT side, 1,702 decks, 122 cards; 100% label agreement with S1 on 78,277 rows; v3val flag = S1's exact 13,761 rows) and `pipeline/model_gen.py` / `train_gen.py` / `eval_gen.py` (GenModel = S1 trunk + card-identity/form embeddings, hand POINTER card head (permutation-equivariant; tray position has no gameplay meaning), identity-conditioned cell head, deck-pointer wait head; eval metric-identical to train_s1.evaluate, verified). 1-epoch 200k-row smoke only: v3val cell .0969 / card .505 / gate_bal .500 -- a pipeline check, not a result. Full training estimate: ~3-5 h (starter, 12-20 epochs, idle GPU), ~9-15 h with the pilot corpus. Before the full run: eval on v3val + a val SAMPLE per epoch (full-val eval dominated the smoke's wall), guard ignore_index=-1 all-dropped batches. RL/live cannot load a gen checkpoint yet (needs a gen loader + decide path).This machine: RTX 5050 Laptop 8 GB (cc 12.0), 32 GB RAM, 16 threads = the gap between RoyaleLearn's `laptop` and `workstation` profiles.
