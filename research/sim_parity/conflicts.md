...
The owner ruling that priced `magic_archer_decoy` 2, `guardienne` 3 and `soul_skeleton` 0.1875 is
headed **"RULING 29"** in `decisions.md`, and is referred to as **"ruling 19"** in this file (four
places), in `gen_pins.py` (`RULING19_PINS`) and in the `source` string of all three pins in
`config/import_pins.json`. There is NO "ruling 19" heading anywhere in decisions.md, so a reader
following the provenance string off a pin finds nothing.

The VALUES are not in doubt -- they agree everywhere and are regenerated by the check added today.
Not renumbered here because that would rewrite three generated `source` strings plus four rows of
