"""ClashAI launcher -- double-click ClashAI.exe (built from this) to open the control panel.

This is the source for the .exe; it does exactly what start_ui.bat does, so the .exe is
...
Build it into ClashAI.exe by double-clicking build_exe.bat in the repo root (installs
...
it finds the repo by its own location, the same way start_ui.bat finds it via %~dp0. Not
...
def _repo_root() -> Path:
    ...
    # same role %~dp0 plays in start_ui.bat.
