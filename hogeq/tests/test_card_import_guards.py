...
class GeneratorReproducesTheCommittedPinsTests(unittest.TestCase):
    """THE PIN THAT LOOKS SET AND IS NOT.

    `config/import_pins.json` is GENERATED by `research/sim_parity/scripts/gen_pins.py`, and
    the file says so in its own meta ("never hand-edit one copy"). Ruling 31a nevertheless
    hand-edited the file and did not touch the generator -- so the next `gen_pins.py` run
    silently reverted the owner value. Nothing failed; the number simply went away. Found
    2026-08-27 while regenerating for ruling 31b, which would have taken the two
    owner-supplied Firecracker spark radii with it.

    These tests are the standing guard: what the generator produces MUST equal what is
    committed, and the values a regeneration would drop are named one at a time so a failure
    says WHICH ruling it just deleted rather than "the files differ".

    NEWLINES ARE NORMALISED BEFORE COMPARISON, AND THAT IS NOT COSMETIC. This repo runs
    `core.autocrlf=true` over an LF index, so a fresh checkout hands you a CRLF working copy
    while the generator writes LF: a byte comparison would fail on a file whose `git diff` is
    empty. That is the trap that already cost a day on `tools/parity_check.py --strict`.
    """

    @staticmethod
    def _gen():
        """Import gen_pins.py by path. Skips when research/ is not in this checkout."""
        sp = ROOT.parents[0] / "research" / "sim_parity"
        path = sp / "scripts" / "gen_pins.py"
        if not path.exists() or not (sp / "ledger" / "stat_diffs.jsonl").exists():
            raise unittest.SkipTest("the pins generator / R2 ledger is not in this checkout")
        spec = importlib.util.spec_from_file_location("_gen_pins_under_test", path)
    ...
    def test_the_generator_reproduces_the_committed_pins(self):
        text, counts = self._gen().render()
        got = (ROOT / "config" / "import_pins.json").read_text(encoding="utf-8")
        self.assertEqual(
            self._norm(text), self._norm(got),
            "gen_pins.py no longer produces the committed import_pins.json. Run "
            "`python research/sim_parity/scripts/gen_pins.py --check` for the per-pin "
            "disagreement. A pin that exists only in the FILE is a pin the next generator "
            "run DELETES -- express it in the generator, never by editing the file.")
        self.assertEqual(counts["total"], len(_pins()))

    def test_check_mode_passes_here_and_bites_on_a_hand_edit(self):
        """The guard's own negative control: a check that cannot fail is not a check.

        Mutates ONE pin value in a COPY (gen.OUTS is repointed at a temp file, the tree is
        never written) and requires --check to exit 1 -- reproducing exactly the ruling-31a
        defect, where the file held a value the generator did not.
        """
        gen = self._gen()
        real = list(gen.OUTS)
        try:
            self.assertEqual(gen.check(), 0, "the committed pins are already stale")
            text, _c = gen.render()
            with tempfile.TemporaryDirectory() as td:
                fake = Path(td) / "import_pins.json"
                doc = json.loads(text)
                doc["pins"][0]["value"] = "@@ NOT WHAT THE GENERATOR SAYS @@"
                fake.write_text(json.dumps(doc, indent=1), encoding="utf-8")
                gen.OUTS = [fake]
                self.assertEqual(gen.check(), 1,
                                 "--check accepted a pins file the generator disagrees with")
        finally:
            gen.OUTS = real

    def test_the_owner_values_a_regeneration_would_have_lost(self):
        """Named one by one, because these are what the silent revert actually costs.

        Each must come out of the GENERATOR, not merely out of the committed file.
        """
        pins = {(p["key"], p["field"]): p
                for p in json.loads(self._gen().render()[0])["pins"]}
        # ruling 31a -- hand-edited into the file, absent from the generator until 2026-08-27
        self.assertEqual(pins[("electro_giant", "reflect_crown_damage")]["value"], 97)
        self.assertNotIn(("electro_giant", "crown_tower_damage"), pins,
                         "ruling 31a RETIRED this field name (it crown-reduced his NORMAL "
                         "swing to 97); the I5 row must not resurrect it")
        # ruling 31b -- OWNER-supplied, table-cell only, so an import emits neither
        self.assertEqual(pins[("firecracker_evo", "spark_radius_large_tiles")]["value"], 2.5)
        self.assertEqual(pins[("firecracker_evo", "spark_radius_tiles")]["value"], 1.2)
        self.assertTrue(pins[("firecracker_evo", "spark_radius_tiles")]["advisory"])
        # the spawned-body elixir prices: without them these bodies fall back to the engine's
        # 4-elixir default through ~30 `spec.elixir` reads in the reward and triage layers
        self.assertEqual(pins[("guardienne", "elixir")]["value"], 3)
        self.assertEqual(pins[("magic_archer_decoy", "elixir")]["value"], 2)
        self.assertEqual(pins[("soul_skeleton", "elixir")]["value"], 0.1875)

    def test_the_pair_is_what_the_generator_wrote_to_both_decks(self):
        """`gen_pins.py` writes BOTH copies; the file's meta promises they are identical."""
        a = (ROOT / "config" / "import_pins.json").read_bytes()
        b = (ROOT.parents[0] / ("hogeq" if ROOT.name == "icebow" else "icebow")
