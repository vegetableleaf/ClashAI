...
class WakeEventTests(unittest.TestCase):
    ...
    def _loop(self, own_cards=("hog_rider", "tesla")):
    ...
    def test_an_unowned_unit_deep_in_our_half_wakes_via_the_veto(self):
        ...
        loop = self._loop(own_cards=("hog_rider", "tesla"))    # no knight in this deck
...
class LeakGuardTests(unittest.TestCase):
    ...
    CARDS = ["tornado", "tesla", "tesla_evo", "ice_wizard", "hog_rider", "rocket", "knight",
             "knight_evo", "the_log", "skeletons"]
    COST = {"tornado": 3, "tesla": 4, "tesla_evo": 4, "ice_wizard": 3, "hog_rider": 4,
            "rocket": 6, "knight": 3, "knight_evo": 3, "the_log": 2, "skeletons": 1}

    def _convert(self, action, na, elixir, hand=("hog_rider", "knight", "the_log", "skeletons"),
                 guard=9.5):
        ...
        if action[0] == 0 and not na and elixir >= guard:
            bow = next((i for i, v in enumerate(hand_vec)
                        if v > 0.5 and self.CARDS[i].replace("_evo", "") == "hog_rider"
    ...
    def test_full_elixir_quiet_wait_becomes_the_bow(self):
        ...
        self.assertEqual("hog_rider", self.CARDS[got[1]])
...
class SpellServingTests(unittest.TestCase):
    ...
    def _tracker(self):
        return TeamTracker(own_cards=["ice_spirit"],
...
class StaticPhantomTests(unittest.TestCase):
    ...
    def _run(self, y=0.62, move=0.0, bars=False, seconds=7.0, own=("hog_rider",)):
