...
def _worker(conn, n_envs: int, seed0: int, drill_frac=None,
    ...
    def payload(i, env, obs, rew=0.0, done=False, outcome=None, pfsp=None,
        ...
        return {
            ...
            # youngest living enemy TROOP's age (s), the gate prior's PRESSURE key (schema 2,
            # sim.ppo_gate_prior_pressure_s); raw age, the parent applies the threshold
            "eage": float(env.enemy_troop_min_age()) if hasattr(env, "enemy_troop_min_age") else 1e9,
