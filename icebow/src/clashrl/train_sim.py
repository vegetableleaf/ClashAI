...
def train_sim(cfg, matches: int = 2000, resume: bool = False, seed: int = 0, envs=None,
    ...
    #
    # MERGE NOTE: the guard is upstream's, the rest is ours and both are load-bearing. It checks
    # `load_path`, not `sim_path`, because this branch resumes from the BEST checkpoint by default
    # -- guarding the wrong file would pass on a stale best and blow up at load_state_dict. And
    # weights_only stays False: torch >= 2.6 flipped that default to True, which refuses these
    # checkpoints outright.
    if (resume and load_path.exists()
            and int(torch.load(load_path, map_location="cpu",
                               weights_only=False).get("in_ch", 3)) != in_ch):
        if not quiet:
            print(f"[train-sim] {load_path.name} has a different observation width (in_ch != {in_ch}) "
