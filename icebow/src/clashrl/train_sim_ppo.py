...
def train_sim_ppo(cfg, matches: int = 2000, resume: bool = False, seed: int = 0, envs=None,
    ...
    #
    # MERGE NOTE: weights_only stays False on both loads (ours). torch >= 2.6 defaults it to True,
    # which refuses these checkpoints -- and here that refusal would surface as the guard raising
    # rather than as a clean "cannot resume", i.e. the crash-loop this guard exists to prevent.
    stale_resume = (resume and ppo_path.exists()
                    and int(torch.load(ppo_path, map_location="cpu",
