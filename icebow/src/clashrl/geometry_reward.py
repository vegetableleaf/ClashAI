...
def score_placement(board: Board, placement: dict) -> Dict[str, float]:
    ...
    threat = pick_threat(board, placement)
    ...
    out["p2_cover"] = _p2_cover(board, eng_pt[0], eng_pt[1], r_body)
    ...
    if is_spell:
        ...
        out["p2_cover"] = _p2_cover(board, px, py, 0.0)     # the pull / blast centre's cover
...
def nonzero_terms(scores: Dict[str, float]) -> List[Tuple[str, float]]:
    ...
    for k in TERM_KEYS:
        v = scores.get(k, 0.0)
        if isinstance(v, (int, float)) and abs(float(v)) > 1e-9 and k != "d_threat":
