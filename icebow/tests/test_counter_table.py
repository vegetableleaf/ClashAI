...
ROWS = [
    {"threat": "balloon", "threat_cards": ["balloon"],
     "respond": [{"card": "tesla", "when": "at_bridge", "where": "center_kite"},
                 {"card": "tornado", "when": "in_our_half", "where": "king_activation"}]},
...
class LookupTests(unittest.TestCase):
    ...
    def test_responses_are_filtered_to_the_hand(self):
        ...
        self.assertEqual("tornado", self.t.best_card(["balloon"],
                                                     hand_bases=["tornado", "knight"]))
    ...
    def test_no_hand_overlap_returns_nothing(self):
        self.assertIsNone(self.t.best_card(["balloon"], hand_bases=["knight", "skeletons"]))
    ...
    def test_the_first_row_for_a_key_wins(self):
        ...
        t = CounterTable([
            {"threat": "override", "threat_cards": ["balloon"],
             "respond": [{"card": "ice_wizard", "when": "at_bridge", "where": "center_kite"}]},
            ROWS[0],
        ])
        self.assertEqual("ice_wizard", t.best_card(["balloon"]))
