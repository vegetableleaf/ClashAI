...
#: This deck (icebow); the hogeq copy of this file swaps its own list in.
OWN = ["knight", "tesla", "x_bow", "skeletons", "the_log",
       "ice_wizard", "rocket", "tornado"]
...
class TestBuildingSidePrior(unittest.TestCase):
    ...
    def test_a_troop_is_not_given_the_building_prior(self):
        """A hog_rider at y=0.55 is in front of deep_mine_y and is NOT a building: the old rules
        apply unchanged and it stays unknown until it moves or bleeds."""
        d = _det("hog_rider", cy=0.55)
        _tracker().tag([d], 0.0)
        self.assertEqual("enemy", d.team)   # hog_rider is not in THIS deck: veto resolves unknown
...
class TestLookalikeRescue(unittest.TestCase):
    def test_marching_wizard_is_our_ice_wizard(self):
        """Motion (rank 2) says ours + 'wizard' is a curated lookalike of ice_wizard ->
        keep 'mine' AND relabel, instead of manufacturing a phantom enemy."""
        tr = _tracker()
        d0 = _det("wizard", cy=0.60)
        tr.tag([d0], 0.0)
        d1 = _det("wizard", cy=0.54)         # net dy -0.06: marching toward the enemy
        tr.tag([d1], 0.5)
        self.assertEqual("mine", d1.team)
        self.assertEqual("ice_wizard", d1.base)
    ...
    def test_the_anchor_rescues_and_relabels_too(self):
        """We just played the Ice Wizard there and the detector says 'wizard': rank-1 ground
        truth + lookalike -> ours, relabelled."""
        tr = _tracker()
        tr.record_play(0.5, 0.6, 0.0, base=None)      # play recorded without a base
        d = _det("wizard", cx=0.5, cy=0.6)
        tr.tag([d], 0.5)
        self.assertEqual("mine", d.team)
        self.assertEqual("ice_wizard", d.base)

    def test_a_lookalike_of_a_card_we_do_not_own_is_not_rescued(self):
        """miner -> mighty_miner is in the table, but this deck holds no mighty_miner: the
        alias map is built from OUR deck, so the rescue never fires."""
        tr = _tracker()
        d0 = _det("miner", cy=0.60)
        tr.tag([d0], 0.0)
        d1 = _det("miner", cy=0.54)
        tr.tag([d1], 0.5)
