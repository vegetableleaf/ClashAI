"""L67m: FirstLight IL_Replay parquet -> a crawl2-shaped directory for ANY pipeline deck, with CARD ALIASES.

Generalises hf_to_crawl.py (icebow-only, exact deck, kept frozen as the record of how corpus_v6/icebow was built).
...
in the deck yaml, and this script only reads it.
...
(tick, card) of the timeline) -- HF tags are anonymised, so the timeline is the only handle.

usage: python scratchpad/gauntlet/L67/hf_to_crawl_deck.py hogeq --hf scratchpad/gauntlet/L67/hf/replays \
           --out scratchpad/gauntlet/ext/crawl_hf_hogeq
"""
...
def load_rule(deck_name: str) -> tuple[frozenset, dict, Path]:
    d = yaml.safe_load((REPO / "pipeline" / "decks" / f"{deck_name}.yaml").read_text(encoding="utf-8"))
...
def our_keys(ours: Path) -> set[tuple]:
...
def main() -> int:
    ...
    a = ap.parse_args()
    want, aliases, ours_dir = load_rule(a.deck)
    ...
    (a.out / "tags_a.json").write_text(json.dumps(tags[0::2]), encoding="utf-8")   # one list per engine slot
    (a.out / "tags_b.json").write_text(json.dumps(tags[1::2]), encoding="utf-8")
