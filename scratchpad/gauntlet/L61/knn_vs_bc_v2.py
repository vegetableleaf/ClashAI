...
DATA = ICEBOW / "data" / "bc_pro_v2"                                        # L61: engine-board dataset
MODELS = DATA / "models"
...
def load_all(device):
    ...
    print(f"[data] N={len(obs)} train={len(tr)} val={len(va)}")
...
def cmd_knn(args):
    ...
    if C_raw.shape[0] <= 9000:                    # L61: skip the full-Gram eigh when n is large
