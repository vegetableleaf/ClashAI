"""L59 gate RERUN (Part 1, tesla + x-bow only) under the L59 module: path-based P1, P2 buildings-only,
P7 swarm-excluded, placement_credit/timing_credit. Copied from L58/gate_replay.py; changes marked L59.
Original header: L58 gate, Part 1: pro replays through the sim, scoring the PRO tile vs the policy's candidate tiles with
...
re-implemented here with the scoring hook (same engine, conversion, slack, tail cap).
    ...
    PYTHONHASHSEED=0 ./.venv/Scripts/python.exe ../scratchpad/gauntlet/L59/gate_replay.py --out ../scratchpad/gauntlet/L59/p1 --limit 20
"""
...
TICK_S = L51.TICK_S
RELEVANT = {"tesla", "x-bow", "skeletons", "knight", "ice-wizard", "tornado", "the-log", "rocket"}   # gate-probe moments (same n as L58)
SCORED = {"tesla", "x-bow"}            # L59: plays scored = buildings only
...
GRADED = ("p1_pull_band", "p1_close_penalty", "p2_cover", "p3_intercept", "p4_spell_frac", "p4_nado",
          ...
          "p1_snapshot", "p1_close_snapshot", "d_path")     # L59: the snapshot twins + d_path (reported, not summed)
SUMMED = GRADED[:10]


def pcred(sc, kind="building"):
    return float(GR.placement_credit(sc, kind))


def tcred(sc):
    return float(GR.timing_credit(sc))
...
def gsum(sc):
    return sum(float(sc[k]) for k in SUMMED)
...
def score_all(board, spec, side, px, py, eng, db=None):
    kw = dict(siege_sight=eng.siege_sight, tower_range=eng.tower_range, king_range=eng.king_range, db=db)   # L59: roles via db
            ...
            if eng.can_afford(row["side"], spec):
                ...
                if slug in RELEVANT:
                    ...
                    if slug in SCORED:
                        ...
                        pro_pc, c_pc = score_all(board, spec, side, x, y, eng, db)
                        pro_ra, c_ra = score_all(board_ra, spec, side, x, y, eng, db)
                        ...
                        for mode, pro, cands in (("pc", pro_pc, c_pc), ("ra", pro_ra, c_ra)):
                            ...
                            # L59: the wired credits (placement_credit for a building; timing_credit = P5)
                            cp, cl = pcred(pro), pcred(cands[locked])
                            rec[f"{mode}_pro_pcred"] = round(cp, 4); rec[f"{mode}_lock_pcred"] = round(cl, 4)
                            rec[f"{mode}_rank_pcred"] = rank_of(cp, [pcred(c) for c in cands.values()])
                            rec[f"{mode}_ntie_pcred"] = sum(1 for c in cands.values() if abs(pcred(c) - cp) < 1e-9)
                            rec[f"{mode}_best_pcred"] = max(cands, key=lambda n: pcred(cands[n]))
                            rec[f"{mode}_pro_tcred"] = round(tcred(pro), 4); rec[f"{mode}_lock_tcred"] = round(tcred(cands[locked]), 4)
                    # gate rule probe: a Tesla at the pros' modal tile vs the corner tile on THIS board (per-card radii)
                    kw = dict(siege_sight=eng.siege_sight, tower_range=eng.tower_range, king_range=eng.king_range, db=db)
                    ...
                    if tm["threat_base"] in GATE_THREATS or tc["threat_base"] in GATE_THREATS:
                        ...
                        g["modal_pcred"] = round(pcred(tm), 4); g["corner_pcred"] = round(pcred(tc), 4)   # L59
                        gate_rows.append(g)
