...
#
# Picks the first seed-0 pool replay whose pro ran OUR card forms (evo knight / evo tesla for icebow --
# 40 of the 477 pool replays are hero-knight games and seed 0's first was one of them), so clips from
# different days are the SAME opening and the difference you see is the model, not the draw. Change
# --seed for a different matchup.
set -e
cd "$(dirname "$0")/.."
DECK=${1:?deck}
CKPT=${2:?checkpoint}
...
.venv/Scripts/python.exe pipeline/engine_play.py "$DECK" --ckpt "$CKPT" --port "$PORT" \
    --matches 1 --seed "$SEED" --gate sample --record-every 2 --own-forms --out "$OUT" > "$OUT/play.out" 2>&1
tail -1 "$OUT/play.out"
